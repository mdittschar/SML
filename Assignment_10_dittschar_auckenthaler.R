...
tab.kmeans
randIndex(tab.kmeans, correct=TRUE, original=TRUE)
...
for (k in 2:6){
  set.seed(1)
  multicutree(hc.average.s,k,data)
}
...
for (k in 2:20){
  set.seed (1)
  km.out.s <- kmeans (data[,3:ncol(data)], k, nstart = 25)
  ks.s= append(ks.s,k)
...
#best k = 11
set.seed(1)
kmeans.out.s= kmeans (data[,3:ncol(data)], 11, nstart = 25)
tab.kmeans.s = table(kmeans.out.s$cluster, factor(data$M))

tab.kmeans.s
randIndex(tab.kmeans.s, correct=TRUE, original=TRUE)
...
for (p in c(5,10,20,50)){
  ...
  title(paste("t-sne (not normalized data), perflexity =",p ))
...
for (p in c(5,10,20,50)){
  ...
  title(paste("t-sne (normalized data), perflexity =",p ))
